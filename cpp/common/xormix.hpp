...
struct xormix {
	...
	static void unmix_partword(word_t &state, word_t &mixin, word_t mixup, const size_t *shifts, size_t nbits) {
		...
		limb_t s1 = right_shift(state, shifts[0] - nbits);
		limb_t s2 = right_shift(state, shifts[1] - nbits);
		limb_t s3 = right_shift(state, shifts[2] - nbits);
		limb_t s4 = right_shift(state, shifts[3] - nbits);
	...
	static void prev(word_t *state, size_t streams) {
		...
		for(size_t s = 0; s < streams; ++s) {
			word_t salted;
			for(size_t ii = 0; ii < L; ++ii) {
				salted.l[ii] = state[0].l[ii] ^ XORMIX_SALTS[s].l[ii];
			}
			mixin[s] = shuffle_word(salted, s, XORMIX_SHUFFLE);
		}
